...
int main()
{
    ...
    // Instantiates satallite
    SatelliteData mySatallite;
    ...
    while (!signalReceived)
    {
        ...
        if (signalReceived) {
            ...
            mySatallite.PartitionDownloadData();
            mySatallite.UpdateAltitude();
            mySatallite.OrbitalCalculation();

            // Prints values to screen
            cout << "Altitude is: " << mySatallite.altitude << endl;
            cout << "Period is: " << mySatallite.orbitalPeriod << endl;

            // Uploads data from rover to satellite
            myRover.CompileData(mySatallite.navCount);
            ...
            while (true) {
                ...
                float wait = mySatallite.orbitalPeriod;

                // Updates time since startup
                
                secondsOn = time(NULL) - start;

                if (abs(secondsOn - mySatallite.orbitalPeriod) < 30) {
